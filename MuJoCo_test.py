...
# Wall-referenced impedance gains.
# K_IMP controls how strongly the brush follows the wall-referenced target.
# D_IMP reduces oscillation and improves smoothness.
# Too large K_IMP or D_IMP can make IK commands aggressive or destabilize contact.
K_IMP = 800.0
D_IMP = 50.0

# Optional batch comparison runs.
RUN_PARAMETER_SWEEP = False
SWEEP_CASES = [
    {"name": "imp_soft", "k_imp": 380.0, "d_imp": 28.0},
    {"name": "imp_mid", "k_imp": 560.0, "d_imp": 40.0},
    {"name": "imp_stiff", "k_imp": 800.0, "d_imp": 50.0},
]
...
CONTROLLER_PRESETS = {
    "baseline": {
        "imp_reengage_gain": IMP_REENGAGE_GAIN,
        "imp_max_reengage_bias": IMP_MAX_REENGAGE_BIAS,
        "imp_max_xcmd_rate": IMP_MAX_XCMD_RATE,
        "x_cmd_max": X_CMD_MAX,
    },
    "strong_reengage": {
        "imp_reengage_gain": 0.00090,
        "imp_max_reengage_bias": 0.0060,
        "imp_max_xcmd_rate": 0.22,
        "x_cmd_max": X_NOMINAL_BASE + 0.020,
    },
}
...
def impedance_position_update(
    ...
    """
    Wall-referenced impedance-inspired x command.

    This forms a virtual spring-damper around a wall-referenced target and maps
    the resulting force mismatch back to a bounded position correction.
    """
    desired_penetration = F_DES / CONTACT_STIFFNESS
    x_ref = contact["wall_surface_x"] - BRUSH_RADIUS + desired_penetration
    if pen_down_elapsed < PEN_DOWN_ENGAGE_TIME:
        engage_ratio = 1.0 - (pen_down_elapsed / max(PEN_DOWN_ENGAGE_TIME, 1e-9))
        x_ref += PEN_DOWN_ENGAGE_BIAS * engage_ratio
    force_deficit = max(0.0, F_DES - force_measured)
    reengage_bias = min(
        controller_params["imp_max_reengage_bias"],
        controller_params["imp_reengage_gain"] * force_deficit,
    )
    if in_recovery:
        reengage_bias += IMP_CONTACT_LOSS_SEARCH_BIAS
    x_ref += reengage_bias
    x_error = x_ref - contact["ee_x"]
    xdot_error = contact["wall_vx"] - ee_vx
    F_imp = k_imp * x_error + d_imp * xdot_error + F_DES
    delta_x_imp = np.clip((F_imp - force_measured) / CONTACT_STIFFNESS, -MAX_DELTA_X, MAX_DELTA_X)
    x_cmd = x_ref + delta_x_imp
    return float(x_cmd), float(x_ref), float(x_error), float(F_imp), float(delta_x_imp)
...
def plot_results(log, output_dir):
    ...
    x_ref = np.asarray(log["x_ref"])
    x_error = np.asarray(log["x_error"])
    delta_x_imp = np.asarray(log["delta_x_imp"])
    x_cmd_minus_ee_x = x_cmd - ee_x
    contact_lost = np.asarray(log["contact_lost"], dtype=float)
    penetration = np.maximum(0.0, ee_x + BRUSH_RADIUS - wall_surface_x)
    ee_y = np.asarray(log["ee_y"])
    ee_z = np.asarray(log["ee_z"])
    pen_down = np.asarray(log["pen_down"], dtype=bool)
    desired_penetration_series = np.where(pen_down, F_DES / CONTACT_STIFFNESS, 0.0)
    ...
    if np.any(np.isfinite(x_ref)):
        axes[0].plot(time, x_ref, label="Impedance reference x_ref", linestyle=":")
    ...
    axes[2].plot(time, x_ref, color="tab:purple", linewidth=2, label="x_ref")
    axes[2].plot(time, x_cmd, color="tab:green", linewidth=1.8, label="x_cmd")
    axes[2].plot(time, x_cmd_minus_ee_x, color="tab:orange", linewidth=1.8, label="x_cmd - ee_x")
    axes[2].set_ylabel("x states [m]")
    axes[2].set_title("Wall-following reference and command gap")
    axes[2].legend()
    axes[2].grid(True, alpha=0.3)

    axes[3].plot(time, penetration, color="tab:brown", linewidth=2, label="Estimated penetration")
    axes[3].plot(time, desired_penetration_series, color="tab:gray", linestyle="--", label="Desired penetration")
    axes[3].set_ylabel("Penetration [m]")
    axes[3].set_title("Brush penetration into wall")
    axes[3].grid(True, alpha=0.3)
    axes[3].legend()

    axes[4].plot(time, contact_lost, color="tab:red", linewidth=1.8, label="contact_lost (1=yes)")
    axes[4].set_xlabel("Time [s]")
    axes[4].set_ylabel("flag")
    axes[4].set_ylim(-0.1, 1.1)
    axes[4].set_title("Contact-loss flag")
...
def run_single_case(
    ...
    k_imp=K_IMP,
    d_imp=D_IMP,
    controller_preset="baseline",
    wall_retreat_distance=None,
    wall_profile_mode=None,
    concave_depth=None,
    concave_sigma=None,
):
    global WALL_RETREAT_DISTANCE, WALL_PROFILE_MODE, CONCAVE_DEPTH, CONCAVE_SIGMA
    ...
    controller_params = CONTROLLER_PRESETS.get(controller_preset, CONTROLLER_PRESETS["baseline"]).copy()

    print(f"Python interpreter: {sys.executable}")
    print(f"Script directory: {SCRIPT_DIR}")
    print(
        f"Running case: {case_name} | mode=impedance | preset={controller_preset} "
        f"| K_IMP={k_imp:.1f} | D_IMP={d_imp:.1f}"
    )
    ...
    try:
        for step in range(num_steps):
            ...
            if pen_down:
                ...
                in_recovery = force_measured < CONTACT_LOSS_FORCE_THRESHOLD
                x_cmd_raw, x_ref, x_error, F_imp, delta_x_imp = impedance_position_update(
                    contact,
                    ee_vx,
                    force_measured,
                    k_imp,
                ...
                x_cmd_max_case = float(controller_params["x_cmd_max"])
                x_cmd_target = float(np.clip(x_cmd_raw, X_CMD_MIN, x_cmd_max_case))
                previous_x_cmd = log["x_cmd"][-1] if len(log["x_cmd"]) > 0 else float(contact["ee_x"])
                rate = float(controller_params["imp_max_xcmd_rate"])
                if in_recovery:
                    rate *= IMP_RECOVERY_RATE_BOOST
                max_step = rate * DT
                x_cmd_rate_limited = np.clip(x_cmd_target, previous_x_cmd - max_step, previous_x_cmd + max_step)
                x_cmd = float((1.0 - IMP_XCMD_LPF_ALPHA) * previous_x_cmd + IMP_XCMD_LPF_ALPHA * x_cmd_rate_limited)
                x_cmd = float(np.clip(x_cmd, X_CMD_MIN, x_cmd_max_case))
            else:
                force_measured = contact["force_used"]
                in_contact = False
                pen_down_elapsed = 0.0
                force_desired = 0.0
                x_cmd = float(np.clip(X_NOMINAL_BASE - PEN_UP_CLEARANCE, X_CMD_MIN, controller_params["x_cmd_max"]))
                x_ref = np.nan
                x_error = np.nan
                F_imp = np.nan
                delta_x_imp = np.nan
            ...
            log["x_ref"].append(x_ref)
            log["x_error"].append(x_error)
            log["F_imp"].append(F_imp)
            log["delta_x_imp"].append(delta_x_imp)
    ...
    force_meas = np.asarray(log["force_meas"])
    x_error_log = np.asarray(log["x_error"])
    x_ref_log = np.asarray(log["x_ref"])
    delta_x_imp_log = np.asarray(log["delta_x_imp"])
    ...
    mean_abs_force_error_pen_down = (
        float(np.mean(np.abs(pen_down_force - F_DES))) if pen_down_force.size > 0 else float("nan")
    )
    min_force_pen_down = float(np.min(pen_down_force)) if pen_down_force.size > 0 else float("nan")
    max_abs_x_error = float(np.nanmax(np.abs(x_error_log))) if np.any(np.isfinite(x_error_log)) else float("nan")
    max_abs_cmd_correction = float(np.max(np.abs(x_cmd_log - ee_x_log))) if x_cmd_log.size > 0 else float("nan")
    max_abs_cmd_delta = float(np.max(np.abs(delta_x_imp_log))) if np.any(np.isfinite(delta_x_imp_log)) else float("nan")
    retreat_mask = pen_down_mask & (np.asarray([wall_motion(ts)[1] for ts in sim_time]) > 0.0)
    x_tracking_delay = x_ref_log[retreat_mask] - ee_x_log[retreat_mask] if np.any(retreat_mask) else np.array([])
    max_x_tracking_delay = float(np.nanmax(x_tracking_delay)) if x_tracking_delay.size > 0 else float("nan")
    ...
    print("\n=== Simulation summary ===")
    print("Controller mode: impedance")
    ...
    print(f"Max x tracking delay after wall retreat: {max_x_tracking_delay:.4f} m")
    print(f"Max |x_error| (impedance): {max_abs_x_error:.4f} m")
    print(f"Max |delta_x_imp| (impedance): {max_abs_cmd_delta:.4f} m")
    ...
    print("- In impedance mode, x_ref follows the moving wall and the spring-damper term improves retreat tracking.")
    ...
    return {
        ...
        "max_x_tracking_delay": max_x_tracking_delay,
        "output_dir": str(output_dir),
    }


def parse_cli_args():
    parser = argparse.ArgumentParser(description="MuJoCo painting demo with impedance control.")
    parser.add_argument("--k-imp", type=float, default=K_IMP)
    parser.add_argument("--d-imp", type=float, default=D_IMP)
    parser.add_argument("--case-name", type=str, default="single")
    parser.add_argument("--sweep", action="store_true", help="Run all cases in SWEEP_CASES.")
    parser.add_argument("--preset", type=str, default="strong_reengage", choices=list(CONTROLLER_PRESETS.keys()))
    parser.add_argument("--compare-contact", action="store_true", help="Run baseline vs strong_reengage comparison.")
    parser.add_argument("--compare-wall", action="store_true", help="Run flat wall vs bumpy wall comparison.")
...
def run_contact_maintenance_comparison():
    print("Running challenging wall-retreat comparison: baseline vs strong_reengage")
    challenging_retreat = max(0.020, WALL_RETREAT_DISTANCE)
    concave_depth = max(0.010, CONCAVE_DEPTH)
    concave_sigma = CONCAVE_SIGMA if CONCAVE_SIGMA > 1e-6 else 0.05

    baseline = run_single_case(
        case_name="baseline_wall_retreat_concave",
        controller_preset="baseline",
        wall_retreat_distance=challenging_retreat,
        wall_profile_mode="concave",
        concave_depth=concave_depth,
        concave_sigma=concave_sigma,
    )
    improved = run_single_case(
        case_name="improved_wall_retreat_concave",
        controller_preset="strong_reengage",
        wall_retreat_distance=challenging_retreat,
        wall_profile_mode="concave",
        concave_depth=concave_depth,
        concave_sigma=concave_sigma,
    ...
    print("\n=== Baseline vs Improved Contact-Maintenance Comparison ===")
    print(f"Scenario: WALL_RETREAT_DISTANCE={challenging_retreat:.4f} m, concave_depth={concave_depth:.4f} m")
    print(
        f"Baseline contact-lost %: {baseline['contact_lost_pct']:.2f} | "
        f"Improved contact-lost %: {improved['contact_lost_pct']:.2f}"
    )
    print(
        f"Baseline longest loss [s]: {baseline['longest_contact_loss_s']:.3f} | "
        f"Improved longest loss [s]: {improved['longest_contact_loss_s']:.3f}"
    )
    print(
        f"Baseline mean |force err| [N]: {baseline['mean_force_error_pen_down']:.3f} | "
        f"Improved mean |force err| [N]: {improved['mean_force_error_pen_down']:.3f}"
    )
    print(
        f"Baseline min force [N]: {baseline['min_force_pen_down']:.3f} | "
        f"Improved min force [N]: {improved['min_force_pen_down']:.3f}"
    )
    print(
        f"Baseline max x-delay [m]: {baseline['max_x_tracking_delay']:.4f} | "
        f"Improved max x-delay [m]: {improved['max_x_tracking_delay']:.4f}"
    )


def run_wall_surface_comparison():
    print("Running wall surface comparison: flat moving wall vs bumpy moving wall")
    flat = run_single_case(
        case_name="flat_wall_surface",
        controller_preset="strong_reengage",
        wall_profile_mode="rigid",
    )
    bumpy = run_single_case(
        case_name="bumpy_wall_surface",
        controller_preset="strong_reengage",
        wall_profile_mode="bumpy",
    )
    print("\n=== Flat vs Bumpy Wall Comparison ===")
    print(
        f"Flat contact-lost %: {flat['contact_lost_pct']:.2f} | "
        f"Bumpy contact-lost %: {bumpy['contact_lost_pct']:.2f}"
    )
    print(
        f"Flat mean |force err| [N]: {flat['mean_force_error_pen_down']:.3f} | "
        f"Bumpy mean |force err| [N]: {bumpy['mean_force_error_pen_down']:.3f}"
    )
    print(
        f"Flat min force [N]: {flat['min_force_pen_down']:.3f} | "
        f"Bumpy min force [N]: {bumpy['min_force_pen_down']:.3f}"
    )
    print(
        f"Flat plots: {flat['output_dir']} | "
        f"Bumpy plots: {bumpy['output_dir']}"
    )
...
def main():
    ...
    args = parse_cli_args()
    do_sweep = bool(args.sweep or RUN_PARAMETER_SWEEP)
    run_default_compare_wall = (
        len(sys.argv) == 1
        and not args.compare_contact
        and not args.compare_wall
        and not do_sweep
    )
    if args.compare_contact:
        SHOW_NATIVE_MUJOCO_VIEWER = False
        KEEP_VIEWER_OPEN_AFTER_SIM = False
        run_contact_maintenance_comparison()
    elif args.compare_wall:
        SHOW_NATIVE_MUJOCO_VIEWER = False
        KEEP_VIEWER_OPEN_AFTER_SIM = False
        run_wall_surface_comparison()
    elif run_default_compare_wall:
        SHOW_NATIVE_MUJOCO_VIEWER = False
        KEEP_VIEWER_OPEN_AFTER_SIM = False
        print("No CLI args detected. Running default flat vs bumpy wall comparison.")
        run_wall_surface_comparison()
    elif do_sweep:
        SHOW_NATIVE_MUJOCO_VIEWER = False
        KEEP_VIEWER_OPEN_AFTER_SIM = False
        print(f"Running parameter sweep with {len(SWEEP_CASES)} cases...")
        for case in SWEEP_CASES:
            run_single_case(
                case_name=case["name"],
                k_imp=float(case["k_imp"]),
                d_imp=float(case["d_imp"]),
            )
    else:
        run_single_case(
            case_name=args.case_name,
            k_imp=float(args.k_imp),
            d_imp=float(args.d_imp),
            controller_preset=args.preset,
        )
