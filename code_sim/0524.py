...
# %% Standalone 0524 task configuration and controller
...
def configure_task(base):
    ...
    # Moving rigid wall. The MuJoCo mocap body is synchronized in the run loop.
    ...
    base.LETTER_STROKE_SPEED = 0.040
    base.CA_STROKE_SPEED_SCALE = 0.50
    base.PEN_UP_TRAVEL_SPEED = 0.18
    base.PRE_STROKE_SETTLE_TIME = 0.20
    base.CORNER_HOLD_C_TIME = 0.08
    base.CORNER_HOLD_A_TIME = 0.08
...
def tool_axis_error(base, current_rotation, target_tool_axis):
    current_tool_axis = current_rotation[:, 2]
    return base.np.cross(current_tool_axis, target_tool_axis)

def ik_pose_with_posture(
    base,
    model,
    ik_data,
    site_id,
    ...
    q_trial = q_seed.copy()
    orientation_length_scale = 0.05
    posture_gain = 0.01
    posture_weights = base.np.array(
        [0.05, 0.05, 0.05, 0.25, 0.50, 1.00],
        dtype=float,
    )

    for _ in range(base.IK_MAX_ITERS):
        ik_data.qpos[qpos_idx] = q_trial
        ik_data.qvel[:] = 0.0
        base.mujoco.mj_forward(model, ik_data)

        current_pos = ik_data.site_xpos[site_id].copy()
        current_rotation = ik_data.site_xmat[site_id].reshape(3, 3).copy()
        position_error = target_pos - current_pos
        orientation_error = tool_axis_error(
            base,
            current_rotation,
            target_tool_axis,
        )
        if (
            base.np.linalg.norm(position_error) < base.IK_TOL
            and base.np.linalg.norm(orientation_error) < 2.0e-3
        ):
            break

        jacp = base.np.zeros((3, model.nv))
        jacr = base.np.zeros((3, model.nv))
        base.mujoco.mj_jacSite(model, ik_data, jacp, jacr, site_id)
        jacobian = base.np.vstack(
            [
                jacp[:, dof_idx],
                orientation_length_scale * jacr[:, dof_idx],
            ]
        )
        task_error = base.np.concatenate(
            [
                position_error,
                orientation_length_scale * orientation_error,
            ]
        )

        regularized = (
            jacobian @ jacobian.T
            + (base.IK_DAMPING**2) * base.np.eye(6)
        )
        jacobian_pinv = jacobian.T @ base.np.linalg.solve(
            regularized,
            base.np.eye(6),
        )
        dq_primary = jacobian_pinv @ task_error
        nullspace = base.np.eye(len(q_trial)) - jacobian_pinv @ jacobian
        posture_step = posture_weights * (q_nominal - q_trial)
        dq = dq_primary + posture_gain * (nullspace @ posture_step)
        dq = base.np.clip(dq, -base.IK_STEP_LIMIT, base.IK_STEP_LIMIT)
        q_trial = base.np.clip(q_trial + dq, joint_lower, joint_upper)
...
def run_impedance_case(base):
    ...
    wall_body_id = base.mujoco.mj_name2id(
        model,
        base.mujoco.mjtObj.mjOBJ_BODY,
        base.WALL_BODY_NAME,
    )
    if wall_body_id < 0:
        raise RuntimeError(f"Wall body '{base.WALL_BODY_NAME}' not found in model.")
    wall_mocap_id = int(model.body_mocapid[wall_body_id])
    if wall_mocap_id < 0:
        raise RuntimeError("The wall body must be a MuJoCo mocap body.")

    initial_wall_x, _ = base.wall_motion(0.0)
    initial_wall_pos = base.np.array(
        [initial_wall_x, 0.0, base.PAINT_Z_CENTER],
        dtype=float,
    )
    initial_wall_quat = base.np.array([1.0, 0.0, 0.0, 0.0], dtype=float)
    data.mocap_pos[wall_mocap_id] = initial_wall_pos
    data.mocap_quat[wall_mocap_id] = initial_wall_quat
    ik_data.mocap_pos[wall_mocap_id] = initial_wall_pos
    ik_data.mocap_quat[wall_mocap_id] = initial_wall_quat
    ...
    target_ee_rotation = data.site_xmat[site_id].reshape(3, 3).copy()
    target_tool_axis = target_ee_rotation[:, 2].copy()
    ...
    fixed_contact_equilibrium = (
        base.WALL_X_CENTER
        - base.WALL_HALF_THICKNESS
        - base.BRUSH_RADIUS
        + desired_penetration
    )
    x_ref_prev = x_imp
    ...
    log_wall_surface, log_contact_gap = [], []
    log_y_nom, log_z_nom, log_y_ee, log_z_ee = [], [], [], []
    log_pen_down = []
    log_orientation_error = []
    log_q = []

    for i in range(steps):
        t = i * base.DT

        wall_x, _ = base.wall_motion(t)
        data.mocap_pos[wall_mocap_id] = base.np.array(
            [wall_x, 0.0, base.PAINT_Z_CENTER],
            dtype=float,
        )
        data.mocap_quat[wall_mocap_id] = initial_wall_quat
        base.mujoco.mj_forward(model, data)
        ...
        x_ref = (
            fixed_contact_equilibrium
            if pen_down
            else fixed_contact_equilibrium - pen_up_clearance
        )
        ...
        if base.USE_QUASI_STATIC_SERVO:
            yz_corr = base.np.zeros(2, dtype=float)
        elif pen_down and letter_idx == 1:
        ...
        log_wall_surface.append(float(wall_surface_x))
        log_contact_gap.append(
            float(wall_surface_x - (ee_pos[0] + base.BRUSH_RADIUS))
        )
        ...
        log_orientation_error.append(
            float(base.np.linalg.norm(tool_axis_error(base, current_rotation, target_tool_axis)))
        )
        log_q.append(data.qpos[qpos_idx].copy())
    ...
    out_dir = base.get_output_dir() / "0524_force_impedance"
    ...
    ax2.plot(log_t, log_xref, "--", label="fixed equilibrium")
    ax2.plot(log_t, log_wall_surface, ":", label="wall surface")
    ...
    retreat_start = base.WALL_MOTION_START + base.WALL_PUSH_DURATION
    retreat_end = retreat_start + base.WALL_RETURN_DURATION
    retreat_mask = (
        (base.np.asarray(log_t) >= retreat_start)
        & (base.np.asarray(log_t) <= retreat_end)
    )
    retreat_wall = base.np.asarray(log_wall_surface)[retreat_mask]
    retreat_ee = base.np.asarray(log_xee)[retreat_mask]
    retreat_force = base.np.asarray(log_f)[retreat_mask]
    retreat_gap = base.np.asarray(log_contact_gap)[retreat_mask]

    fig_follow = base.plt.figure(figsize=(10, 7))
    follow_x = fig_follow.add_subplot(2, 1, 1)
    follow_x.plot(log_t, log_wall_surface, label="wall surface")
    follow_x.plot(
        log_t,
        base.np.asarray(log_xee) + base.BRUSH_RADIUS,
        label="brush front",
    )
    follow_x.set_ylabel("X [m]")
    follow_x.grid(True, alpha=0.3)
    follow_x.legend()

    follow_gap = fig_follow.add_subplot(2, 1, 2)
    follow_gap.plot(log_t, 1000.0 * base.np.asarray(log_contact_gap), label="contact gap")
    follow_gap.axhline(0.0, color="black", linestyle="--", linewidth=1.0)
    follow_gap.set_xlabel("Time [s]")
    follow_gap.set_ylabel("Gap [mm]")
    follow_gap.grid(True, alpha=0.3)
    follow_gap.legend()
    fig_follow.tight_layout()
    follow_path = out_dir / "wall_retreat_following.png"
    fig_follow.savefig(follow_path, dpi=160)
    base.plt.close(fig_follow)
    ...
    print(f"Wall following figure: {follow_path}")
    ...
    print(
        "Mean tool-axis drift: "
        f"{base.np.degrees(base.np.mean(log_orientation_error)):.2f} deg"
    )
    print(
        "Max tool-axis drift: "
        f"{base.np.degrees(base.np.max(log_orientation_error)):.2f} deg"
    )
    q_range_deg = base.np.degrees(base.np.ptp(base.np.asarray(log_q), axis=0))
    print(
        "Wrist joint ranges (q4/q5/q6): "
        f"{q_range_deg[3]:.2f} / {q_range_deg[4]:.2f} / {q_range_deg[5]:.2f} deg"
    )
    metrics = {
        "planned_duration_s": float(local_plan["total_duration"]),
        "simulation_duration_s": float(sim_time),
        "video_fps": int(base.RENDER_FPS),
        "desired_force_n": float(base.F_DES),
        "mean_yz_error_pen_down_mm": mean_err_mm,
        "max_yz_error_pen_down_mm": max_err_mm,
        "mean_tool_axis_drift_deg": float(
            base.np.degrees(base.np.mean(log_orientation_error))
        ),
        "max_tool_axis_drift_deg": float(
            base.np.degrees(base.np.max(log_orientation_error))
        ),
        "wrist_joint_ranges_deg": {
            "q4": float(q_range_deg[3]),
            "q5": float(q_range_deg[4]),
            "q6": float(q_range_deg[5]),
        },
    }
    force_arr = base.np.asarray(log_f)
    force_des_arr = base.np.asarray(log_fdes)
    if base.np.any(pen_down_arr):
        force_error_pen = force_arr[pen_down_arr] - force_des_arr[pen_down_arr]
        metrics.update(
            {
                "mean_force_pen_down_n": float(base.np.mean(force_arr[pen_down_arr])),
                "force_mae_pen_down_n": float(base.np.mean(base.np.abs(force_error_pen))),
                "force_rmse_pen_down_n": float(
                    base.np.sqrt(base.np.mean(force_error_pen**2))
                ),
                "force_std_pen_down_n": float(base.np.std(force_arr[pen_down_arr])),
                "peak_force_pen_down_n": float(base.np.max(force_arr[pen_down_arr])),
                "minimum_force_pen_down_n": float(base.np.min(force_arr[pen_down_arr])),
            }
        )

    if retreat_wall.size > 1:
        wall_retreat_mm = 1000.0 * float(retreat_wall[-1] - retreat_wall[0])
        brush_follow_mm = 1000.0 * float(retreat_ee[-1] - retreat_ee[0])
        contact_ratio = 100.0 * float(base.np.mean(retreat_gap <= 0.0))
        mean_retreat_force = float(base.np.mean(retreat_force))
        follow_ratio = 100.0 * brush_follow_mm / wall_retreat_mm
        metrics.update(
            {
                "wall_retreat_mm": wall_retreat_mm,
                "brush_follow_mm": brush_follow_mm,
                "brush_follow_ratio_pct": follow_ratio,
                "retreat_contact_maintained_pct": contact_ratio,
                "mean_retreat_force_n": mean_retreat_force,
                "retreat_force_rmse_n": float(
                    base.np.sqrt(base.np.mean((retreat_force - base.F_DES) ** 2))
                ),
                "maximum_retreat_gap_mm": float(1000.0 * base.np.max(retreat_gap)),
                "minimum_retreat_gap_mm": float(1000.0 * base.np.min(retreat_gap)),
            }
        )
        print(f"Wall retreat distance: {wall_retreat_mm:.2f} mm")
        print(f"Brush follow distance: {brush_follow_mm:.2f} mm")
        print(f"Brush follow ratio: {follow_ratio:.1f}%")
        print(f"Retreat contact maintained: {contact_ratio:.1f}%")
        print(f"Mean retreat force: {mean_retreat_force:.2f} N")
